...
function fillCat (element){ //TODO define jquery function to replace the long lines
  ...
  if($(element).hasClass("coursCat")) {
    $(element).closest('table').next('table').find('.recapCours').html(""+(Math.round((parseFloat($(element).closest('table').next('table').find('.recapCours').html())-calendars[choice][8]) * 10) / 10));
  } else if($(element).hasClass("projetTutCoursCat")) {
    $(element).closest('table').next('table').find('.recapProjetTutUniv').html(""+(Math.round((parseFloat($(element).closest('table').next('table').find('.recapProjetTutUniv').html())-calendars[choice][8]) * 10) / 10));
  } else if($(element).hasClass("projetTutEntrepriseCat")) {
    $(element).closest('table').next('table').find('.recapProjetTutEts').html(""+(Math.round((parseFloat($(element).closest('table').next('table').find('.recapProjetTutEts').html())-calendars[choice][8]) * 10) / 10));
  } else if($(element).hasClass("entrepriseCat")) {
    $(element).closest('table').next('table').find('.recapEntreprise').html(""+(Math.round((parseFloat($(element).closest('table').next('table').find('.recapEntreprise').html())-calendars[choice][8]) * 10) / 10));
  }
